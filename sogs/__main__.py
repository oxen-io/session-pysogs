...
with db.pool as conn:
    if args.add:
        if args.room:
            conn.execute("INSERT INTO rooms(token, name, description) VALUES(?1, ?1, ?1)", [args.room])
        if args.mod:
            conn.execute("INSERT INTO users(session_id, moderator, visible_mod) VALUES(?, TRUE, TRUE)", [args.mod])
        if args.admin:
            conn.execute("INSERT INTO users(session_id, moderator, admin) VALUES(?, TRUE, TRUE)", [args.admin])
    if args.remove:
        if args.room:
            conn.execute("DELETE FROM rooms WHERE token = ?", [args.room])
        if args.mod:
            conn.execute("UPDATE users WHERE session_id = ? SET moderator=FALSE visible_mod=FALSE", [args.mod])
        if args.admin:
