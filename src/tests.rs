...
async fn test_session_version_endpoint() {
    ...
    assert_eq!(handlers::SESSION_VERSIONS.lock().unwrap().clone()["ios"].1, ios);
    let android = handlers::get_session_version("android").await.unwrap();
    assert_eq!(handlers::SESSION_VERSIONS.lock().unwrap().clone()["android"].1, android);
    let desktop = handlers::get_session_version("desktop").await.unwrap();
    assert_eq!(handlers::SESSION_VERSIONS.lock().unwrap().clone()["desktop"].1, desktop);
}
