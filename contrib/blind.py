...
if len(sys.argv) < 3:
    print(f"Usage: {sys.argv[0]} SERVERPUBKEY {{SESSIONID|\"RANDOM\"}} [SESSIONID ...] -- blinds IDs", file=sys.stderr)
    sys.exit(1)
...
for i in range(len(sids)):
    if sids[i] == "RANDOM":
        sids[i] = "05" + nacl.signing.SigningKey.generate().verify_key.to_curve25519_public_key().encode().hex()
    if len(sids[i]) != 66 or not sids[i].startswith('05') or not all(c in '0123456789ABCDEFabcdef' for c in sids[i]):
...
for s in sids:
    ...
    if s[0] == 0x05:
        k25 = sodium.crypto_core_ed25519_scalar_reduce(nacl.hash.blake2b(s[1:] + server_pk, digest_size=64, encoder=RawEncoder))
