...
if len(sys.argv) < 3:
    print(f"Usage: {sys.argv[0]} SERVERPUBKEY SESSIONID [SESSIONID ...] -- blinds IDs", file=sys.stderr)
    sys.exit(1)
...
for s in sids:
    if len(s) != 66 or not s.startswith('05') or not all(c in '0123456789ABCDEFabcdef' for c in s):
