...
def test_blinded_key_derivation(seed_hex, blinded15_id_exp, blinded25_id_exp):
    ...
    k25 = sodium.crypto_core_ed25519_scalar_reduce(blake2b([b'\x05' + s.verify_key.to_curve25519_public_key().encode(), fake_server_pubkey_bytes], digest_size=64))
    ...
    assert (crypto.compute_blinded25_key_from_15(bytes.fromhex(blinded15_id[2:]), _server_pk=fake_server_pubkey_bytes).hex()
            ==
            blinded25_id[2:])
...
def test_blinded_transition(
    ...
    with config_override(REQUIRE_BLIND_KEYS=True, REQUIRE_BLIND_V2=False):
        ...
        r = client.get(
            '/capabilities',
            headers=x_sogs(user.ed_key, crypto.server_pubkey, 'GET', '/capabilities', **x_sogs_blind),
        )
        ...
        assert room.get_mods(global_admin) == ([get_blinded_id(mod)], [get_blinded_id(admin)], [], [])
